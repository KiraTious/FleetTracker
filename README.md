...
3. **Построить и поднять контейнеры** (создастся БД `fleettracker` в контейнере):
   ...
   docker compose up -d
   ```
   Контейнер web при старте сам накатывает миграции и применяет SQL-скрипт `seed/seed_data.sql`, который создаёт демо-данные и аккаунты.

4. **Проверить, что сервис работает**
   - Откройте http://localhost:5000/ или http://localhost:5000/login — появится форма входа.
   - Готовые учётные записи: `admin/admin`, `manager/manager`, `driver_alex/driver1`, `driver_maria/driver2`.
