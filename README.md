...
3. **Построить и поднять контейнеры** (создастся БД `fleettracker` в контейнере):
   ...
   docker compose up -d
   ```
   Контейнер `db` собирается из кастомного образа PostgreSQL 17 с предзагруженным схемой `db/schema.sql` и данными `seed/seed_data.sql`. Контейнер `web` при старте дополнительно запускает ORM-подстраховку из `seed_db.py`, чтобы убедиться, что демо-аккаунты и связанный транспорт присутствуют.

4. **Проверить, что сервис работает**
   - Откройте http://localhost:5000/ или http://localhost:5000/login — появится форма входа.
   - Готовые учётные записи: `admin/admin`, `manager/manager`, `driver_alex/driver_alex`, `driver_maria/driver_maria`.
   - После входа вы попадёте в интерфейс своей роли; админ/менеджер могут добавлять записи через веб.

5. **Остановить сервис при необходимости**
   ```bash
   docker compose down
   ```
...
Фронтенд разбит на отдельные страницы (`/login`, `/portal/admin`, `/portal/manager`, `/portal/driver`) и отправляет запросы к тем же эндпоинтам, что упрощает проверку API без внешних инструментов.
...
# Миграции сохраняются в репозитории, но при работе с кастомным контейнером БД схема уже создана init-скриптами.
flask db upgrade
